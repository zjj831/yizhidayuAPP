...
<section class="content-header">
	...
	<div class="box">
		<div class="box-header">
			<a class=" btn btn-primary pull-right" href="<?php echo U('Coupon/addcategory');?>">添加分类</a>
		</div>
		<div class="box-body" >
			<?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><div class="container-fluid">
				...
				<?php if(is_array($item['sonInfo'])): $i = 0; $__LIST__ = $item['sonInfo'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub): $mod = ($i % 2 );++$i;?><div id="<?php echo ($sub["id"]); ?>" class="row list_two" style="height:50px;line-height:50px;font-size:20px;">
						<span style="border:none;background:#fff;box-sizing: border-box;" class="col-sm-10"><?php echo ($sub["title"]); ?></span>
...
<script src="<?php echo C('PUBLIC');?>/js/form/trashall.js" type="text/javascript" charset="utf-8"></script>
	...
	<script>
		$('.del2').on("click",function(){
            ...
            $.ajax({
                url:"<?php echo U('Coupon/deletecate');?>",
                data:{id:id},
                dataType:"json",
				type:"post",
