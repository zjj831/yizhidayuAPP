...
<div class="box">
	<div class="box-header">
		<a class="btn btn-primary pull-right" href="<?php echo U('Worthbuy/excel');?>">批量添加</a>
		<form action="<?php echo U('Worthbuy/index');?>"  method="get" class="form-inline">
...
<table id="example1" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
						...
						<?php if(is_array($datas)): foreach($datas as $key=>$item): ?><tr role="row">
								...
								<td><?php echo ($item["catename"]); ?></td>
									<td><?php echo ($item["startdate"]); ?>-<?php echo ($item["enddate"]); ?></td>
									<td>
					<a href="<?php echo U('Worthbuy/index_home');?>?id=<?php echo ($item["id"]); ?>"><img class="myicon" style="display: inline;" src="<?php echo C('PUBLIC');?>/icon/bianji2.png"></img>  </a>
...
$(window).on('resize', centerModals);  
	...
	if (!ifdel) {
	 ...
	 $.get("<?php echo U('Worthbuy/index_destroy');?>?id="+id,function(data,status){
if(data=="ok"){
		alert("删除成功");
 	location.reload();
}else{
